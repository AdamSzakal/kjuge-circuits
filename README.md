...
| ⦿ **Every block, always** | All 542 blocks are on the map at all times. An *All blocks* switch says which is loud — your circuit, or everything — and the walk never leaves the screen. |
...
| 🟢 **How far you are round** | A stop's dot carries a green ring: part of the way round for some of its boulders sent, the whole way round for all of them. |
...
- **How far you are round a stop** — a stop's dot carries a ring in the done colour: a part of the way round for some of its boulders ticked, the whole way round for all of them, and no ring at all for a stop you have not started. The ring is the dash pattern of a circle, so it costs one shape and no drawing of its own, and its ends are cut square or a round cap would run the arc past the share it stands for.
