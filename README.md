...
| ⦿ **Browse every block** | One button on the map drops the pick and draws all 542 blocks in grey. A second tap brings back exactly the circuit you had. |
...
- **Browse from the map** — the button under the crosshair drops the pick and draws every block of the dataset in grey, and a second tap brings back exactly the circuit that was there, merged lists included. It is the thing you do most often at the crag — read the block you are standing at — and it used to cost four taps through the sheet, the *Circuits* tab and *None*, with the way back left to you. The pick it dropped is written down beside the pick itself, because browsing is a valid pick and a reload would otherwise come back to it with nothing left to say where you were; with nothing remembered the way back is the default circuit. The button follows what the map drew and not what you tapped, so picking *None* in the sheet lights it up just the same.
