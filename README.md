...
- **Add to a circuit** — the foot of the filters offers the set back: *+ Add these 32 to a circuit*. The count is read the same way the map reads it, so the button and the walk header can never disagree — a pick is the lists and circuits you chose, and no pick at all is the whole dataset drawn behind them. It opens a picker of your circuits plus a new one; the circuit it fills is switched on, and a new one opens in the editor because it still wants a name.
