...
| ⦿ **Every block, always** | All 542 blocks are on the map at all times. One button says which is loud — your circuit, or everything — and the walk never leaves the screen. |
| 👆 **Nothing out of reach** | Two blocks a few metres apart used to fight over your thumb. A tap now opens everything under it, on one page. |
...
- **Every block, always** — the map no longer chooses between your circuit and the crag. Every block of the dataset is drawn behind the circuit at all times, and the button under the crosshair only says which of the two is loud: the background goes from faint specks to full grey, while the circuit keeps its colour, its numbers and its walking line in both. Nothing is dropped, so nothing has to be found again. With no circuit picked there is nothing to turn down, and the button says so and goes quiet.
- **Nothing out of reach** — every dot used to carry an invisible circle to catch its own taps. Where two blocks stand a few metres apart those circles lay on top of each other and the upper one took every tap, so the block underneath could not be opened at all, however carefully you aimed. There are no catching circles now: a tap is matched to the nearest dot, and everything within a thumb's width of it comes along. The cluster is never drawn — it only decides what the tap opens — so two blocks under one thumb open as one page carrying both, each under its own name and its stop number. A crowd is a different problem, and while there is zoom left a tap on one pulls it apart instead.
