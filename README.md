...
| 🗺️ **Merge circuits** | Tick several lists and they become one walk, shared blocks counted once. |
| ⦿ **Every block, always** | All 542 blocks are on the map at all times. The *All blocks* switch says which is loud — your circuit, or everything — and the walk never leaves the screen. |
| 👆 **Nothing out of reach** | Blocks drawn on top of each other used to fight over your thumb. A tap now zooms in until they stand apart, then opens the one you meant. |
| ◍ **How far you are round** | A stop's dot carries a ring: part of the way round for some of its boulders sent, the whole way round for all of them. |
| ‹ › **Along the walk** | The stop page steps to the next stop and back, in walking order, without going to the map for it. |
| 🎚️ **Filter** | Area, grade, 13 characteristics and star rating, applied to the built-in lists and to your own circuits alike. Each one folds shut, so the whole page is five words and a button. |
| 🔍 **Read the topo** | The big picture pinches, zooms and drags, and steps between the block's topos with a swipe or the arrows. |
...
- **100 boulders to a circuit** — a circuit is a day out, and a day is not a thousand boulders: the biggest built-in list is 100, the walk is routed by a 2-opt that grows with the square of the stops, and every id rides in the share link. So 100 is the ceiling, and `addToCircuit` is the only door — every way in goes through it. The editor header counts against it (*84 of 100*), the circuit picker shows the room left and dims a circuit that has none, and nothing is ever dropped in silence: a tap that cannot land says *"… is full — 100 boulders is the limit"*, a pasted list reports what did not fit apart from what was already there, and a shared circuit longer than 100 keeps its first 100 and says so.
- **Circuits** — a tab of its own: multi-select checkboxes over the built-in lists (community first, then the top-lists and the guidebook styles) and your own circuits. Selecting several merges their boulders, de-duplicates shared sectors, and recomputes one combined walking route. With nothing picked the map shows **every block of the area filter** in grey instead of going empty — 1547 boulders over 244 blocks as it opens, or all 3141 over 542 with every area on — with no route, because nobody picked an order. The view stays where you were looking, so the blocks appear around it; tap one to read it, and pick a circuit to get the walk back. A list that crosses several crags is counted in crags, not in kilometres, because its route is a visiting order and not a footpath. The picks of the last visit come back on the next one (`localStorage`), browsing included; only a first visit takes the default pick, and a link that carries its own lists always wins over both. The list numbers a share link carries belong to the data, not to the order the tab shows, so reordering the groups never moves somebody else's link.
- **Filters** — the third tab: area, grade, character and star rating, applied to everything drawn on the map, the built-in lists *and* your own circuits. Every section folds: shut to start with, so the whole tab fits a phone at once, and a shut section keeps a dot beside its name while it is narrowing something, so it can never hide that it is working. What you open stays open on the next visit. Picking circuits and filtering their contents never change each other. The tab carries a dot while anything is narrowed. Grade chips cover the whole dataset scale (3 … 8C, plus ungraded); a boulder of a circle that a filter removes is marked *filtered out* in the editor.
- **Area filter** — a chip per crag, biggest first, which puts Kjugekull (1547 of the 3141 boulders) at the head. **The map opens on Kjugekull alone**: it is half the dataset and the reason this exists, and a first visit that opened on 3141 boulders spread over 40 km of road would not be an answer to anything. The other 13 crags are one tap away, and **Reset** comes back to Kjugekull rather than to everything. An area is a place you drive to: every block of a crag is within a walk of its own parking — at Kjugekull the farthest is 781 m — and the next crag is kilometres away, so "which crags" is the same question as "where am I climbing today". It is the widest of the four cuts, so it is asked first. Kjugekull alone leaves 244 blocks; it also turns the two cross-crag *Around Ivösjön* lists from a 95 km driving tour into one crag you can walk. Areas travel in the link as crag slugs, not numbers, so a crag added later cannot move somebody else's link onto a different forest. A link that says nothing about areas means *every* area — that is what every link made before this filter existed meant, and two of the lists are not at Kjugekull at all.
- **Add to a circuit** — the foot of the filters offers the set back: *+ Add these 32 to a circuit*. The count is read the same way the map reads it, so the button and the walk header can never disagree — a pick is the lists and circuits you chose, and no pick at all is the whole dataset drawn behind them. It opens a picker of your circuits plus a new one; the circuit it fills is switched on, and a new one opens in the editor because it still wants a name. Added, already there and would not fit are three different outcomes, and the toast names whichever ones happened.
...
- **Reading a topo close up** — a topo is where the lines are, and the whole picture at once is too small on a phone to follow one. The big picture zooms and moves: pinch it, double tap it, or roll the wheel, and drag it about once it is bigger than the frame. Zooming holds still whatever is under your fingers, and the picture can never be pushed off its own edge. A block with several topos steps between them — the arrows either side, a sideways swipe while the picture is whole, the arrow keys, and a count under it saying where you are. Each picture opens whole, because the zoom belongs to the one being read and not to the block.
- **Share** — the current selection (lists + area, grade and character filters) is encoded in the URL hash; the **Share** button copies a link that reopens the exact same circuit. No backend required.
- **Base map** — OpenTopoMap: contour lines and forest tracks for the walk-in. Free for light use, no API key.
- **Every block, always** — the map no longer chooses between your circuit and the crag. Every block of the dataset is drawn behind the circuit at all times, and the **All blocks** switch beside the crosshair only says which of the two is loud: the background goes from faint specks to full grey, while the circuit keeps its colour, its numbers and its walking line in both. Nothing is dropped, so nothing has to be found again. It is a switch and not a button, because it is a state that stays and not an action that happens, and it carries its name so nothing has to be guessed from a symbol. With no circuit picked there is nothing to turn down, and the switch says so and goes quiet.
- **Nothing out of reach** — every dot used to carry an invisible circle to catch its own taps. Where two blocks stand a few metres apart those circles lay on top of each other and the upper one took every tap, so the block underneath could not be opened at all, however carefully you aimed. There are no catching circles now: a tap is matched to the nearest dot, and any dot drawn on top of that one competes for it. Where several compete the tap does not guess and does not stack their pages — it takes the map in until they stop covering each other, and the next tap is unambiguous. Far enough is exactly that and no more: the moment the dots are clear of one another, not a comfortable gap around them. Two stops 2 px apart come out 29 px apart, which is four levels — and four is the least a doubling per level allows from 2 px. The map goes past 17 for it, where there is no new tile to fetch and the last one is stretched: blurry, but still a map, and the only thing that can separate two blocks a metre apart. Offline saving keeps to the tiles that really exist, so none of this costs anything at the crag. Two blocks on the very same spot are past what any zoom can do, and there a page carrying both is the only way either can be opened at all.
- **How far you are round a stop** — a stop's dot carries a ring: a part of the way round for some of its boulders ticked, the whole way round for all of them, and no ring at all for a stop you have not started. It was drawn in the green a tick is everywhere else in the app, which is also one of the colours a circuit can be — on *Kjugekull | Hidden gems* it was green on green and could not be seen. It is drawn in black or white now, whichever the dot's own colour can carry, worked out from the same relative luminance a contrast ratio is built from, so it holds for every circuit and for any colour a circuit of your own is given. A faint full ring under it is the part still to do, so a short arc reads as a share and not as a smudge, and the ring sits inside the dot so only the fill has to be beaten and never the map as well.
- **The map keeps its place under the sheet** — the stop page stands over the lower half of the screen, so a stop tapped low down used to go behind it the moment it opened, and the map appeared to jump. The map moves with the sheet now: the stop keeps the place it had, measured against the part of the map you can still see, so a stop halfway down the screen is halfway down what is left of it. Closing gives back exactly the same shift and the map ends where it began — the lift is remembered rather than worked out twice, because the sheet changes height as you step along the walk and a shift undone by a different amount would leave the map adrift. Stepping to the next stop lands it in the middle of the same visible strip, and a stop opened from the walk list, which is not on the screen to begin with, is brought into that strip instead of being kept anywhere.
- **Along the walk, from the stop** — the stop page carries **‹** and **›**, which step to the next stop and back in walking order and bring the map with them. Reading a stop and then hunting the next dot is the wrong way round at the crag, when the walk already has an order. A stop with no place in that order — a block off the circuit, or two blocks opened together — has no next, and the buttons stand away rather than lead nowhere.
- **Fast enough for an old phone** — the 542 background blocks are drawn on one canvas, not as 542 shapes in the page, and they are built once per set of filters and then only restyled. So changing what is loud builds nothing: it went from 21 ms and 1084 shapes in the page to 0.6 ms and one canvas, and a redraw after a pan costs half a millisecond. A block page — its photographs, its boulders, its links — is written the first time somebody opens that block, instead of for all 542 of them on every redraw.
